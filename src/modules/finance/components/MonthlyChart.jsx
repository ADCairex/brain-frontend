...
export default function MonthlyChart({ data: propData, hideAmounts = false }) {
    ...
    return (
        <div className="bg-white dark:bg-slate-700/60 rounded-2xl p-6 border border-slate-100 dark:border-slate-600 shadow-sm h-full">
            <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Resumen Mensual</h3>
            <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} barGap={8}>
                        ...
                        <YAxis
                            ...
                            tickLine={false}
                            tick={{ fill: '#64748b', fontSize: 12 }}
                            tickFormatter={(value) => hideAmounts ? '***' : `${value / 1000}k €`}
                        />
                        <Tooltip
                        ...
                        <Bar dataKey="expenses" fill="#f43f5e" radius={[6, 6, 0, 0]} name="Gastos" />
                    </BarChart>
                </ResponsiveContainer>
            </div>

...
export default function AddInvestmentModal({
  ...
  const netInvested =
    parseFloat(totalPaid) > 0 ? parseFloat(totalPaid) - commissionVal : 0;
  const derivedPricePerUnit =
    netInvested > 0 && parseFloat(quantity) > 0
  ...
  const handleSave = async () => {
    ...
    if (!totalPaid || parseFloat(totalPaid) <= 0)
      errs.totalPaid = "Ingresa el importe pagado";
    if (!quantity || parseFloat(quantity) <= 0)
      errs.quantity = "Ingresa la cantidad recibida";
    ...
    try {
      await createInvestment({
        ...
        source_account_id:
          sourceAccountId && sourceAccountId !== "none"
      ...
      <DialogContent className="sm:max-w-md overscroll-contain">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-slate-900 dark:text-white">
            {prefill
              ? `Nueva compra — ${prefill.asset_name}`
              : "Nueva Inversión"}
          </DialogTitle>
        </DialogHeader>
